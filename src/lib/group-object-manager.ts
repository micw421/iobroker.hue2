...
export class GroupObjectManager {
    ...
    public async updateResource(resources: ResourceManager, resource: HueResource): Promise<void> {
        if (resource.type === 'scene') {
            await this.updateSceneStatus(resource);
            await this.updateActiveScene(resources, resource);
            return;
        }
    ...
    private async syncGroupedContainerType(
        ...
        for (const container of resources.getByType(type)) {
            ...
            const groupedLight = this.getServiceReferences(container)
                ...
                .find((candidate): candidate is HueResource => candidate !== undefined);
            ...
            await this.adapter.extendObjectAsync(baseId, {
                ...
                common: { name },
                native: {
                    hueResourceId: container.id,
                    hueResourceType: container.type,
                    groupedLightResourceId: groupedLight?.id,
                },
            });
            await this.createInfoState(`${baseId}.name`, 'Name', name);
            await this.createSimpleStringState(
                `${baseId}.active_scene`,
                'Active scene',
                this.getActiveSceneName(resources, container.id, container.type),
            );
    ...
    private async syncScenesForGroup(
        baseId: string,
        groupId: string,
        groupType: string,
        resources: ResourceManager,
        ...
        for (const scene of scenes) {
            ...
            await this.adapter.extendObjectAsync(sceneBaseId, {
                ...
                common: { name },
                native: {
                    hueResourceId: scene.id,
                    hueResourceType: scene.type,
                    groupResourceId: groupId,
                    groupResourceType: groupType,
                },
            });
            ...
            if (status !== undefined) {
                await this.createState(`${sceneBaseId}.status`, {
                    name: 'Status',
                    type: 'string',
                    role: 'text',
                    value: status,
    ...
    private async updateSceneStatus(scene: HueResource): Promise<void> {
        ...
        const group = this.asRecord(scene.group);
        const groupId = this.asString(group?.rid);
        const groupType = this.asString(group?.rtype);
        if (!groupId) return;

        const root = groupType === 'room' ? 'rooms' : groupType === 'zone' ? 'zones' : undefined;
        if (!root) return;
    ...
    private async updateActiveScene(resources: ResourceManager, scene: HueResource): Promise<void> {
        ...
        const root = groupType === 'room' ? 'rooms' : 'zones';
        const activeScene = this.getActiveSceneName(resources, groupId, groupType);
        await this.adapter.setStateAsync(`${root}.${groupId}.active_scene`, activeScene, true);
    }
    ...
    private getActiveSceneName(resources: ResourceManager, groupId: string, groupType: string): string {
        for (const scene of resources.getByType('scene')) {
            const group = this.asRecord(scene.group);
            if (this.asString(group?.rid) !== groupId || this.asString(group?.rtype) !== groupType) continue;

            const status = this.getSceneStatus(scene);
            if (status === undefined || status === 'inactive') continue;

            const metadata = this.asRecord(scene.metadata);
            return this.asString(metadata?.name) ?? scene.id;
        }
        return '';
    }

    private getSceneStatus(scene: HueResource): string | undefined {
        const status = this.asRecord(scene.status);
        return this.asString(status?.active);
    }
    ...
    private async syncGroupedLightStates(baseId: string, resource: HueResource): Promise<void> {
        ...
        if (typeof on?.on === 'boolean') {
            await this.createState(`${baseId}.on`, {
                name: 'On', type: 'boolean', role: 'switch', value: on.on, resource, write: true,
            });
        }

        const dimming = this.asRecord(resource.dimming);
        if (typeof dimming?.brightness === 'number') {
            await this.createState(`${baseId}.dimming`, {
                name: 'Dimming', type: 'number', role: 'level.dimmer', value: dimming.brightness,
                unit: '%', min: 0, max: 100, resource, write: true,
            });
        }
        ...
        if (Object.prototype.hasOwnProperty.call(resource, 'color_temperature')) {
            ...
            await this.createState(`${baseId}.color_temperature`, {
                ...
                value: typeof colorTemperature?.mirek === 'number' ? colorTemperature.mirek : undefined,
                unit: 'mired',
        ...
        if (Object.prototype.hasOwnProperty.call(resource, 'color')) {
            ...
            await this.createState(`${baseId}.color`, {
                name: 'Color',
                type: 'string',
                role: 'text',
                value: typeof xy?.x === 'number' && typeof xy?.y === 'number'
                    ? JSON.stringify({ x: xy.x, y: xy.y })
                    : '',
                resource,
                write: true,
            });
    ...
    private async updateGroupedLightValues(baseId: string, resource: HueResource): Promise<void> {
        ...
        const dimming = this.asRecord(resource.dimming);
        if (typeof dimming?.brightness === 'number') {
            await this.adapter.setStateAsync(`${baseId}.dimming`, dimming.brightness, true);
        }

        const colorTemperature = this.asRecord(resource.color_temperature);
        if (typeof colorTemperature?.mirek === 'number') {
            await this.adapter.setStateAsync(`${baseId}.color_temperature`, colorTemperature.mirek, true);
        }
    ...
    private async createSimpleStringState(id: string, name: string, value: string): Promise<void> {
        await this.adapter.extendObjectAsync(id, {
            type: 'state',
    ...
    private async createInfoState(id: string, name: string, value: string): Promise<void> {
        await this.createSimpleStringState(id, name, value);
    }
