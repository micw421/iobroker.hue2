...
/** Creates rooms, zones and scenes from Hue API v2 resources. */
export class GroupObjectManager {
    ...
    public async sync(resources: ResourceManager): Promise<void> {
        ...
        await this.syncScenes(resources);
    }
    ...
    private async syncScenes(resources: ResourceManager): Promise<void> {
        await this.adapter.delObjectAsync('scenes', { recursive: true });
        await this.adapter.extendObjectAsync('scenes', {
            type: 'folder',
            common: { name: 'Hue scenes' },
            native: {},
        });
        ...
        for (const scene of resources.getByType('scene')) {
            ...
            await this.adapter.extendObjectAsync(baseId, {
                ...
                native: {
                    ...
                    groupResourceId: this.asString(group?.rid),
                    groupResourceType: this.asString(group?.rtype),
                },
            });

            await this.createInfoState(`${baseId}.name`, 'Name', name);
            await this.createState(`${baseId}.recall`, {
                name: 'Recall',
                type: 'boolean',
                role: 'button',
                value: false,
    ...
    private async syncGroupedLightStates(baseId: string, resource: HueResource): Promise<void> {
        ...
        if (typeof colorTemperature?.mirek === 'number') {
            ...
            await this.createState(`${baseId}.color_temperature`, {
                name: 'Color temperature', type: 'number', role: 'level.color.temperature',
                value: colorTemperature.mirek, unit: 'mired', min: this.asNumber(schema?.mirek_minimum),
                max: this.asNumber(schema?.mirek_maximum), resource, write: true,
            });
        }
        ...
        if (typeof xy?.x === 'number' && typeof xy?.y === 'number') {
            await this.createState(`${baseId}.color`, {
                name: 'Color', type: 'string', role: 'text',
                value: JSON.stringify({ x: xy.x, y: xy.y }), resource, write: true,
            });
        }
    }
    ...
    private async updateGroupedLightValues(baseId: string, resource: HueResource): Promise<void> {
        const on = this.asRecord(resource.on);
        if (typeof on?.on === 'boolean') {
            await this.adapter.setStateAsync(`${baseId}.on`, on.on, true);
        }
