...
export class ObjectManager {
    ...
    private async updateEntertainmentStateForDevice(resources: ResourceManager, device: HueDeviceResource, createObject: boolean): Promise<void> {
        ...
        const id = `devices.${device.id}.active_entertainment`; const active = this.getActiveEntertainmentName(resources, light.id, entertainmentServices);
        if (createObject) await this.adapter.extendObjectAsync(id, { type: 'state', common: { name: 'Active entertainment', type: 'string', role: 'text', read: true, write: false }, native: { hueDerivedFrom: 'entertainment_configuration', hueLightResourceId: light.id } });
        await this.adapter.setStateAsync(id, active, true);
        const legacyId = `devices.${device.id}.entertainment_active`; if (await this.adapter.getObjectAsync(legacyId)) await this.adapter.delObjectAsync(legacyId);
    }
    ...
    private getActiveEntertainmentName(resources: ResourceManager, lightId: string, entertainmentServices: HueResource[]): string { const ids = new Set(entertainmentServices.map(s => s.id)); const config = resources.getByType('entertainment_configuration').find(c => c.status === 'active' && (this.hasReference(c.light_services, lightId, 'light') || this.containsEntertainmentServiceReference(c.channels, ids))); if (!config) return ''; const metadata = this.asRecord(config.metadata); return this.asString(metadata?.name) ?? config.id; }
